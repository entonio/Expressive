...
private class Reader {
    ...
    private func expression(_ tokens: [Token]) throws -> Expression {
        ...
        return Expression(
            lhs: try expression(lhs),
            op: op,
            rhs: try expression(rhs)
        )
    }

    private func expression(_ token: Token) throws -> Expression {
        switch token {
    ...
    private func associate(_ tokens: [Token]) throws -> [Token] {
        if tokens.count == 1 {
            return tokens
        }
    ...
    private func associate(_ tokens: [Token], _ ops: [Op], _ rightAssociative: Bool) -> [Token] {
    ...
    private func group(_ tokens: [Token], _ ops: [Op]) -> [Token] {
        var tokens = tokens
        for (index, token) in tokens.enumerated() {
            if index % 2 == 0 {
                if case .nested(let inner) = token {
    ...
    private func normalize(_ tokens: [Token]) throws -> [Token] {
        position = 0
        ...
        /*
         if case let .variable(chars) = tokens.first {
            return .variable(String(chars))
        }
        fatalError()
         */
    ...
    private func readTokens(of string: Substring, until delimiter: Character? = nil) throws -> (tokens: [Token], rest: Substring) {
        ...
        while true {
            ...
            if c == delimiter {
                ...
                if case .nuggle(_) = currentToken {
                    throw StringExpressionError("Unexpected character '\(c)' when trying to read number. Context: [\(context)] starting from \(count(string))")
    ...
    private func count(_ string: Substring) -> Int {
